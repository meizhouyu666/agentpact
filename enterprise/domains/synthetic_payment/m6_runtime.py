"""Synthetic-only M6 coordinator from installation through trusted compilation."""
...
class SyntheticM6TrustedContext(BaseModel):
    ...
    workload_principal_id: str = "synthetic_m6_planner_service"

    @model_validator(mode="after")
    def validate_identity_scope(self) -> "SyntheticM6TrustedContext":
        if self.user.org_id != self.tenant_id:
            raise ValueError("Trusted user tenant does not match the M6 tenant")
        return self


class SyntheticM6Compilation(BaseModel):
...
class SyntheticM6ExecutionBinding(BaseModel):
...
class SyntheticM6PermitBinding(BaseModel):
...
def build_synthetic_installation(
    *,
    tenant_id: str,
...
def compile_synthetic_request(
    *,
    ...
    if len(active.installations) != 1:
        raise ValueError("Synthetic M6 requires one accepted active Domain Pack installation")
    ...
    resolved_grants = CapabilityResolver(registry).resolve(
        CapabilityResolutionContext(
            ...
            revocation_epoch="synthetic-m6-epoch-1",
            purpose="synthetic_m6_constrained_planning",
        )
    )
    grants = CapabilityGrantSet(
        grants=[
    ...
    if projected.capability_id != CAPABILITY_ID:
        raise ValueError("Synthetic M6 Planner selected an unsupported Capability")
    ...
    contract = TaskContract(
        ...
        success_criteria=["Independent synthetic result probe confirms one submission"],
    ...
    work_order = ExecutionWorkOrder(
        ...
        contract_id=context.contract_id,
        grant_id=grant.grant_id,
        navigation_goal="Submit the approved synthetic payment exactly once through the governed locator path",
...
def append_execution_trace(
    ...
    *,
    ...
    permit_binding: SyntheticM6PermitBinding,
    attempt_id: str,
    attempt_task_id: str,
...
def bind_compilation_for_execution(
    ...
    *,
    ...
    payment_id = str(observed["payment_id"])
    if payment_id not in compilation.business_plan.data_scope.resource_ids:
        raise ValueError("Compiled payment input is outside the trusted BusinessPlan scope")
    idempotency_key_digest = _canonical_digest(f"synthetic:{payment_id}")
    ...
    return SyntheticM6ExecutionBinding(binding_digest=_canonical_digest(values), **values)
...
def bind_permit_to_execution(
    ...
    *,
    ...
    return SyntheticM6PermitBinding(
        execution_binding_digest=execution_binding.binding_digest,
        work_order_id=execution_binding.work_order_id,
        task_id=task_id,
...
def _require_live_compilation(compilation: SyntheticM6Compilation, *, now: datetime) -> CapabilityGrant:
...
def _normalize_business_inputs(value: dict[str, object]) -> dict[str, object]:
    unknown_input_fields = set(value) - PaymentFacts.model_fields.keys()
    if unknown_input_fields:
        raise ValueError(f"Synthetic payment inputs contain unknown fields: {sorted(unknown_input_fields)}")
    return PaymentFacts.model_validate(value).model_dump(mode="json")
