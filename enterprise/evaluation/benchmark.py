"""Generic offline benchmark metrics and fault-replay helpers."""

from collections import Counter

from pydantic import BaseModel, Field

from enterprise.governance.recovery import ExecutionFailureEvent, RecoveryDecision, decide_recovery
...
def summarize(records: list[BenchmarkRecord]) -> BenchmarkMetrics:
    if not records:
        raise ValueError("Benchmark requires at least one record")
    count = len(records)
