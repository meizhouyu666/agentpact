...
async def lifespan(fastapi_app: FastAPI) -> AsyncGenerator[None, Any]:
    ...
    recovery_task = start_approval_recovery_scheduler()
    if recovery_task:
        LOG.info("Approval recovery scheduler started")
    ...
    await stop_approval_recovery_scheduler()
...
def create_api_app(*, agent_run_composition: AgentRunComposition | None = None) -> FastAPI:
    ...
    阅读指引：
    1. start_forge_app() 负责初始化 Skyvern 的全局运行时依赖。
    2. include_router(...) 暴露原生 Skyvern 的 API 入口。
    3. enterprise 项目再在上面叠加中间件和扩展路由。

    如果你想知道“后端真正从哪里开始”，就从这个函数看起。
    """
    ...
    # These are the original Skyvern API entrypoints. Requests created here
    # eventually flow into task/workflow execution and then into ForgeAgent.
    ...
    # Everything below this point is the enterprise extension layer.
    # Skyvern remains the execution engine; enterprise/* adds governance.
    #
    # The middleware injects tenant visibility rules before route handlers run.
