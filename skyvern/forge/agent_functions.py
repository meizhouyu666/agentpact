...
class AgentFunction:
    async def validate_step_execution(
        ...
        # A Phase 2 approval recovery deliberately reuses the paused Step.
        # Agent execution moves it from RESUMING to RUNNING before scraping a
        # fresh page, so no pre-approval Action is replayed.
        has_valid_step_status = step.status in [StepStatus.created, StepStatus.failed, StepStatus.resuming]
