...
class Settings(BaseSettings):
    ...
    # Phase 2 Action Governance.  `off` preserves Phase 1 behavior; `audit`
    # records redacted candidate-action observations without blocking execution.
    GOVERNANCE_MODE: str = "off"
    GOVERNANCE_AUDIT_HMAC_SECRET: str | None = None
    GOVERNANCE_MODEL_EGRESS_ALLOWLIST: list[str] = []
    ENABLE_GOVERNANCE_RECOVERY_SCHEDULER: bool = False
    ...
    @classmethod
    def validate_governance_mode(cls, value: str) -> str:
        """Reject incomplete enforce mode rather than silently bypassing permits."""

        mode = value.lower().strip()
        if mode not in {"off", "audit", "enforce"}:
            raise ValueError("GOVERNANCE_MODE must be one of: off, audit, enforce")
        if mode == "enforce":
            raise ValueError(
                "GOVERNANCE_MODE=enforce is not available yet: the production Domain Pack, live runtime wiring, "
                "owner decisions, and scoped rollback approval remain incomplete. "
    ...
    @classmethod
    def validate_governance_recovery_settings(cls, value: int) -> int:
        if value <= 0:
            raise ValueError("Governance recovery interval and batch size must be positive")
        return value

    @field_validator("ENABLE_GOVERNANCE_RECOVERY_EXECUTION")
    @classmethod
    def validate_governance_recovery_execution(cls, value: bool, info: Any) -> bool:
        if value and info.data.get("GOVERNANCE_MODE") != "enforce":
            raise ValueError("Governance recovery execution requires sealed enforce mode")
        return value
