...
class Action(BaseModel):
    ...
    # Action 是所有动作的统一内部协议。
    # 无论前面是 Skyvern 自己的 DOM-based prompt，还是后面兼容的 CUA / UI-TARS，
    # 最终都要落到这套结构上，执行器和数据库只认这个边界。
    ...
    def validate(cls: Type[T], value: Any) -> T:
        if isinstance(value, dict):
            ...
            # 这里相当于一个“动作工厂”：
            # 外部拿到的可能还是普通 dict，但只要 action_type 确定，
            # 就会分派到对应的 Pydantic 子类做强校验。
...
class WebAction(Action):
    # WebAction 强制要求 element_id，表示这类动作必须落在某个页面元素上。
    # click / input / select 这些动作天然依赖页面定位，所以与控制流动作分层。
    element_id: Annotated[str, Field(coerce_numbers_to_str=True)]


class DecisiveAction(Action):
    # DecisiveAction 表示“改变任务走向”的决策类动作，
    # 典型就是 COMPLETE / TERMINATE，它们不是在操作页面，而是在改写状态机。
