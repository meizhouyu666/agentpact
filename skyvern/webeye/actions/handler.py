...
from skyvern.governance_guard import has_complete_governed_execution_context
...
class ActionHandler:
    ...
    async def _handle_governed_action(
        *,
        ...
        from enterprise.governance.execution_guard import verify_execution_authorization
        ...
        verify_execution_authorization(
            ...
            page_html=scraped_page.html,
            hmac_secret=settings.GOVERNANCE_AUDIT_HMAC_SECRET,
        )
        require_allowed_profile(
            effect=execution_authorization.effect,
            profile=execution_profile,
        )
        require_cua_execution_evidence(
            profile=execution_profile,
            evidence=cua_execution_evidence,
            action_fingerprint=execution_authorization.action_fingerprint,
            observation_hash=execution_authorization.observation_hash,
        )
        async with app.DATABASE.Session() as session:
            attempt = await authorize_execution_attempt(
                db_session=session,
                permit_id=execution_authorization.permit_id,
                action_fingerprint=execution_authorization.action_fingerprint,
                observation_hash=execution_authorization.observation_hash,
                idempotency_key=execution_authorization.idempotency_key,
                effect=execution_authorization.effect,
                execution_profile=execution_profile,
                cua_execution_evidence=cua_execution_evidence,
            )
        ...
        async with app.DATABASE.Session() as session:
            await mark_execution_attempt_executing(db_session=session, attempt_id=attempt.attempt_id)
            await session.commit()

        try:
            with governed_execution_profile(execution_profile):
                results = await ActionHandler._handle_action_ungoverned(
                    scraped_page=scraped_page,
                    task=task,
                    step=step,
    ...
    async def _mark_governed_attempt_unknown(attempt_id: str) -> None:
        from enterprise.governance.execution_attempt_service import mark_execution_attempt_unknown

        async with app.DATABASE.Session() as session:
            await mark_execution_attempt_unknown(
                db_session=session,
                attempt_id=attempt_id,
                error_message="Browser execution completed; business result probe is pending",
            )
            await session.commit()

    @staticmethod
    @traced()
    async def _handle_action_ungoverned(
        scraped_page: ScrapedPage,
        task: Task,
        step: Step,
        page: Page,
        ...
        # 这是单个强类型 Action 的执行边界。
        #
        # 进入这里之前：
        # - 模型已经给出了动作计划
        # - 这个计划已经被解析成具体的 Action 对象
        #
        # 在这里，Skyvern 开始做确定性的浏览器动作：
        # click / input / select / download 等。
        ...
        if not trigger_download_action:
            # 大部分动作都比较直接：执行 -> 收集 ActionResult -> 持久化动作记录。
        ...
        # Download-triggering actions are special because the side effect continues
        # after the click/select returns. Skyvern therefore watches the download
        # directory and only marks success when a new file actually appears.
    ...
    async def _handle_action(
        ...
        try:
            if action.action_type in ActionHandler._handled_action_types:
                # 真正执行前先做一次动作合法性检查。
                # 例如：一个 WebAction 指向了当前 scraped_page 中不存在的 element_id，
                # 那就不该继续往 Playwright 层打，直接在这一层返回失败结果。
                ...
                if setup := ActionHandler._setup_action_types.get(action.action_type):
                    # setup / handler / teardown 把“一个动作”拆成了可扩展的三段生命周期。
                    # 这样后面如果某类动作需要前置准备或后置收尾，不用污染主执行逻辑。
...
def check_for_invalid_web_action(
    ...
    # 这一步是执行器入口的护栏：
    # 模型也许给了一个“看起来合法”的动作对象，但只要它引用的元素不在当前页面快照里，
    # 就要在这里拦下，而不是让更底层的 locator / click 再随机报错。
...
async def handle_input_text_action(
    ...
    if skip_context_parsing:
        ...
        # 这里不是为了“直接决定填什么值”，而是为了判断这个输入框是什么类型：
        # 普通文本框、搜索框、地址联想框、日期框、验证码框……
        # 这一步决定后面走普通 input 逻辑，还是进入 mini-agent 的自动补全/下拉选择逻辑。
    ...
    if (
        ...
        # 这里是一个非常“agentic”的探测动作：
        # 不是先盲目输入，而是先监听 DOM 增量变化，再按一次 ArrowDown 试探是否会弹出候选项。
        # 如果页面因此冒出新的可交互元素，系统就会把它当成“可联想/可选择输入框”处理。
        has_onclick_attr = await skyvern_element.has_attr("onclick", mode="static")
        await skyvern_element.scroll_into_view()
        ...
        if len(incremental_element) == 0:
            ...
            # 一旦观测到增量元素，就说明这个输入框不是“单纯填字”那么简单，
            # 后面会转入 sequentially_select_from_dropdown，让模型从新增候选项里继续决策。
...
async def handle_select_option_action(
    ...
    try:
        ...
        if (
            ...
            await skyvern_element.scroll_into_view()
            if execution_mechanism_is_allowed(ExecutionMechanism.COORDINATE):
...
async def handle_complete_action(
    ...
    # COMPLETE 不是模型一句“我做完了”就直接通过。
    # Skyvern 会再走一次 complete_verify，看当前页面状态是否真的满足用户目标，
    # 所以 COMPLETE 本质上是“候选完成结论”，不是立即生效的最终裁决。
...
# 这里是动作类型到执行函数的注册表。
# 前面 parse_actions 产出的强类型 Action，到这一步靠 action_type 找到对应 handler，
# 这就是“动作对象”和“浏览器执行逻辑”之间的桥。
...
async def input_or_auto_complete_input(
    ...
    # 这是一层“小回路”：
    # 先拿原始输入值尝试触发联想；
    # 不行的话，再让次级模型生成若干更可能命中的候选值逐个试。
    # 也就是说，大模型在这里不是直接点击，而是在帮系统“改写搜索词/提示词”。
...
async def sequentially_select_from_dropdown(
    ...
    MAX_SELECT_DEPTH = 3
    # 这里支持“连续选择”，不是只点一次就结束。
    # 像省/市/区三级联动、日期选择器翻月，都会反复经历：
    # 抓新增元素 -> 让模型判断下一步 -> 再执行下一步。
...
async def select_from_dropdown(
    ...
    # 注意这里送给模型的，不是整页原始 DOM，而是“这次交互之后新增/变化出来的候选区域”。
    # 这能大幅缩小决策范围，让模型专注在刚弹出的 dropdown / listbox / suggestions 上。
...
async def _get_input_or_select_context(
    ...
    # 这里其实是在做“字段语义识别”：
    # 判断当前控件是地址、日期、搜索、普通文本还是验证码输入，
    # 后面的执行策略几乎都建立在这一次语义分类之上。
