...
def parse_action(
    ...
    # 这里做的是“模型输出 -> 内部强类型动作”的第一次收敛。
    # LLM 返回的只是一个松散 JSON，只有先映射到 Skyvern 自己的 Action 类，
    # 后面的执行器才能用统一接口做校验、执行、审计和失败处理。
    if "id" in action:
    ...
    # prompt 中给模型看的 element id 只是当前页面快照下的临时编号。
    # 真正执行时，系统会把这个编号再映射回抓取阶段保存的 element hash / element data，
    # 这样后续动作处理器既能定位元素，也能保留完整上下文用于回放与日志。
    ...
    if not action_type.is_web_action() and action_type != ActionType.SCROLL:
        ...
        # 这一步很重要：不是所有动作都应该绑定页面元素。
        # 像 WAIT / COMPLETE / TERMINATE 本质上是控制流动作，如果错误携带 element_id，
        # 会污染缓存计划和后续执行判断，所以这里主动剥离。
...
def parse_actions(
    ...
    for idx, action in enumerate(json_response):
        try:
            ...
            # 这里补齐的是“运行时元数据”，不是模型生成内容。
            # 也就是说同一个 click / input 动作，只有挂上 task_id、step_id、action_order，
            # 才真正变成平台里可追踪、可审计、可重放的一次执行指令。
