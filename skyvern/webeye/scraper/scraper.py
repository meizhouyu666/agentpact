...
async def scrape_website(
    ...
    ************************************************************************************************
    ...
    #
    # Skyvern 不只需要截图，还需要：
    # - screenshots：给模型做视觉 grounding
    # - interactable DOM tree：给动作执行做精确元素引用
    # - page text / HTML：给推理和抽取补充更多上下文
    #
    # 这个包装函数负责重试，并把失败统一整理成主循环能处理的形式。
...
async def scrape_web_unsafe(
    ...
    """
    # 这个函数会把一个活的 Playwright 页面转换成 agent 可推理的“世界状态”。
    # 它的输出就是 ScrapedPage。
    ...
    # 第三步：把所有感知结果打包成一个统一快照对象，交给后续主循环使用。
    return ScrapedPage(
        elements=elements,
...
async def get_interactable_element_tree(
    ...
    """
    # 这是 Skyvern 感知系统里偏 DOM 的那一半。
    # 每个可交互元素都会打上一个 Skyvern 自己生成的 ID，
    # 这样后续模型输出引用的是稳定的逻辑元素，而不是脆弱的 XPath。
...
class IncrementalScrapePage(ElementTreeBuilder):
    ...
    async def get_incremental_element_tree(
        ...
        # 这里抓的不是“整页快照”，而是用户上一步动作之后新冒出来的增量元素。
        # 典型场景就是：点击下拉框后弹出的 option 列表、输入地址后出现的联想候选。
        # 这份增量树会被单独送给 custom-select prompt，让模型只在新增候选里做判断。
    ...
    async def start_listen_dom_increment(self, element: ElementHandle | None = None) -> None:
        # 在真实交互前先挂一个前端观察器，持续记录“这次动作导致了哪些新 DOM 变化”。
        # 这就是 Skyvern 能做 dropdown / autocomplete 的基础设施。
