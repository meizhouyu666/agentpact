    ...
    <>
      ...
      <div className="flex items-center justify-between border-b border-amber-500/30 bg-amber-500/10 px-4 py-2.5">
        <div className="flex min-w-0 flex-1 items-center gap-2">
          ...
          <p className="truncate text-xs" style={{ color: "var(--finrpa-text-secondary)" }}>
            <span style={{ color: "var(--finrpa-text-primary)" }}>{label}</span> needs 2FA
          </p>
          {timeRemaining !== null && (
            <span
              className={`ml-2 flex items-center gap-1 text-xs font-medium ${
                isTimeCritical ? "text-red-300" : ""
              }`}
              style={isTimeCritical ? undefined : { color: "var(--finrpa-text-muted)" }}
            >
