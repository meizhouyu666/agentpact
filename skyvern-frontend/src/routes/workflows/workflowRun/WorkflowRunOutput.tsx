  ...
  return (
    <div className="space-y-5">
      {webhookFailureReasonData ? (
        <div className="rounded bg-slate-elevation2 p-6">
          <div className="space-y-4">
            <h1 className="text-lg font-bold">{t("tasks.webhookFailureReason")}</h1>
      ...
      {activeBlock ? (
        <div className="rounded bg-slate-elevation2 p-6">
          <div className="space-y-4">
            <h1 className="text-lg font-bold">{t("workflows.blockOutputs")}</h1>
            {showFailureReason ? (
              <div className="space-y-2">
                <h2>{t("tasks.failureReason")}</h2>
              ...
              <div>{t("workflows.blockNoOutputs")}</div>
            )}
          </div>
        </div>
      ) : null}
      {observerOutput ? (
        <div className="rounded bg-slate-elevation2 p-6">
          <div className="space-y-4">
            <h1 className="text-lg font-bold">{t("workflows.task20Output")}</h1>
            <CodeEditor
              language="json"
      ...
      <div className="rounded bg-slate-elevation2 p-6">
        <div className="space-y-4">
          <h1 className="text-lg font-bold">{t("workflows.workflowRunOutputs")}</h1>
          <CodeEditor
            language="json"
      ...
      <div className="rounded bg-slate-elevation2 p-6">
        <div className="space-y-4">
          <h1 className="text-lg font-bold">{t("workflows.workflowRunDownloadedFiles")}</h1>
          <div className="space-y-2">
            {fileUrls.length > 0 ? (
              fileUrls.map((url) => {
              ...
              <div>{t("workflows.noFilesDownloaded")}</div>
            )}
          </div>
        </div>
      </div>
